...
public class HelloWorldAdvanced {
	...
	public static void main(final String[] args) {
		...
		try{
			...
			final PDFViewerPreferences viewerPrefs = PDFViewerPreferences.newInstance(pdfDocument);
			...
			final PDFPageTree pdfPageTree = pdfDocument.requirePages();
			final PDFPage pdfPage = pdfPageTree.getPage(0);
			 ...
			 */
			final PDFContents pdfContents = PDFContents.newInstance(pdfPage.getPDFDocument());
			...
			final ContentWriter contentWriter = ContentWriter.newInstance(
			...
			*/
			...
			final Instruction gSave = InstructionFactory.newGSave();
			final Instruction gRestore = InstructionFactory.newGRestore();
			...
			*/
			 ...
			 */
			final Content content = contentWriter.close();
			...
			final File file = new File(FIL‎E_PATH);
			file.mkdirs();
			if (file.exists()) {
				file.delete();
			}
			...
			final RandomAccessFile outputPDFFile = new RandomAccessFile(FIL‎E_PATH, "rw");
	...
	private static double[] getColorComponents(final Color color) {
		final float[] colorComponents = color.getColorComponents(null);
		final double[] colorComponentsDouble = new double[colorComponents.length];
